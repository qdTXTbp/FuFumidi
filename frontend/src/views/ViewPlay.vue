  ...
  <div class="page">
    <div class="page-head">
      ...
      <div v-if="stats" class="row" style="gap:8px;flex-wrap:wrap">
        ...
        <span class="tag">{{ stats.bars }} 小节</span>
        <span class="tag">{{ stats.notes }} 音符</span>
    ...
    <template v-else>
      ...
      <div class="grid" style="margin-top:16px;grid-template-columns:minmax(0,1.6fr) minmax(0,1fr)">
        <!-- 轨道混音 -->
        <div class="card">
          <div class="card-title"><span class="dot"></span>轨道混音器</div>
          <div class="mix-track" v-for="(tr, i) in state.tracks" :key="i">
            ...
            <div style="width:70px;display:flex;align-items:center;gap:6px" :title="'声像 ' + (tr.pan > 0 ? 'R' + Math.round(tr.pan * 100) : tr.pan < 0 ? 'L' + Math.round(-tr.pan * 100) : '中')">
          ...
          <div v-if="!state.tracks.length" class="muted small">该曲目没有可混音的轨道</div>
        </div>

        <!-- 曲目信息 -->
        <div class="card">
          <div class="card-title"><span class="dot" style="background:var(--accent2)"></span>曲目概览</div>
          <div style="display:flex;flex-direction:column;gap:10px">
            <div class="spread" v-for="(row, i) in [
              { k: '曲名', v: currentSong?.name },
              { k: '速度', v: stats?.bpm + ' BPM' },
              { k: '拍号', v: stats?.sig },
              { k: '小节数', v: stats?.bars },
              { k: '音符数', v: stats?.notes },
              { k: '时长', v: stats?.dur },
              { k: '轨道数', v: state.tracks.length },
            ]" :key="i">
