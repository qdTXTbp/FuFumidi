...
const CHANGELOG = {
  '3.1.22': [
    '修复：收藏按钮不可见 / 点击无反馈——图标库缺失 heart/heart-o 导致「♥ 收藏」按钮渲染为空；已补齐图标并新增「已加入/已取消收藏」提示',
    '修复：Aria-AMT 钢琴无法安装运行时——依赖组误判模块（amt/ariautils）且 aria-amt 声明的 torchaudio<=2.5 会强制降级 torch；现按真实模块检查并用 --no-deps 安装避免降级',
    '修复：Aria-AMT 转录实际不可用——调用方式与真实 CLI（aria-amt transcribe <variant> <ckpt> -load_path -save_dir）不匹配，已改为真实 CLI 调用、自动定位权重、CUDA 下开启 -compile',
    '修复：补全模型依赖弹窗一直显示「正在检查…」——安装进行中改为「正在安装…」并逐组实时反馈进度',
    '修复：转录页参数预设应用无效——后端预设为空/加载异常时静默留空，现已兜底内置预设并给出「已应用预设」提示',
    '修复：大 MIDI 分析失效——和弦检测由 O(小节×音符) 改为近线性按覆盖小节累计，密度/时长分析设上限，超大 MIDI 也能正常出结果',
    '改进：转录输出与输入文件同源——未指定输出时 MIDI 默认保存到音频所在目录（配置「默认输出目录」则优先），目录不可写自动回退临时目录',
    '改进：依赖安装提速——镜像顺序改为国内快源优先（清华→阿里→官方），pip 短超时+低重试，不再长时间卡在不可达源',
  ],
  '3.1.21': [
  ...
  '3.1.18': [
    '修复：完整性检查 core-corrupt 误报（根治）——Electron 对 app.asar 路径做了虚拟化拦截，fs.statSync(app.asar).size 恒返回 0，导致「文件过小」判断每次启动都误报核心文件损坏。现改用未补丁的 original-fs 读取真实文件大小，配合「落盘稳定」等待，彻底消除误报',
  ],
  '3.1.17': [
    '修复：完整性检查 core-corrupt 误报——全新安装/更新后启动偶发提示核心文件损坏。原逻辑读取到安装器替换文件瞬间的「正在写入、尚未写完」状态即判损坏，现改为先等待文件落盘稳定（连续采样大小一致）再校验，彻底规避写入中误判',
  ],
  '3.1.16': [
