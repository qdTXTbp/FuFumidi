...
const CHANGELOG = {
  '3.3.1': [
    '修复：3.3.0 升级后转录模型无法下载（报 ENOENT）——安装器替换内置模型目录后，指向旧目录的 junction 变成断链，现启动时与每次下载前自动清理断链占位，下载自动落盘用户目录',
    '修复：内置小文件模型（basic-pitch 量化模型等）改随迁移复制到用户目录，不再依赖 junction 透出，升级后依旧可用',
  ],
  '3.3.0': [
    '新增：专业音效调节——10 段均衡器 + 流行/摇滚/古典等 7 种预设 + 低音增强与空间声效果，MIDI 与音频曲目共用同一条效果链',
    '新增：播放模式（顺序/随机/单曲循环/列表循环）与无缝播放——播放期间预解析下一首，曲尾淡出 + 曲首淡入平滑衔接',
    '新增：断点续播、书签（记录/跳转进度）与睡眠定时（15/30/60/90 分钟，到点自动淡出停止）',
