...
3.3.1 要点：修复 3.3.0 升级后转录模型无法下载（ENOENT）——安装器替换内置模型目录后残留断链 junction，现启动与下载前自动清理；内置小文件模型改为复制到用户目录，升级后依旧可用。3.3.0 全部内容（音效调节 / 播放模式 / 无缝衔接 / 智能音乐库 / 托盘自启 / 并行转录 / 乐谱零重刻等）保持不变。
