...
当前版本线：**4.0.1**（[发布说明](https://github.com/qdTXTbp/FuFumidi/releases/tag/v4.0.1)）。

4.0.1 要点：云同步改为增量上传，只上传有改动的曲目（未改动时直接提示无需上传）；同步过程增加进度与已用时显示；修复转录生成的 MIDI 无法同步、以及本地曲库备份迁移可能丢失 MIDI 字节的问题；音色库新增 8 款与手机端 FuMiVoice 对齐的音色。
