...
当前版本线：**4.1.0**（[发布说明](https://github.com/qdTXTbp/FuFumidi/releases/tag/v4.1.0)）。

4.1.0 要点：修复部分机器上转录永久失败的问题（启动内置 Python 时继承了系统的 `PYTHONPATH`，加载到系统里另一份旧 numpy，报 `No module named 'numpy.exceptions'`）；依赖自检改为真实导入验证，可识别并强制重装修复「已损坏」的包；新增设置 → 功能「清理失效曲目」；歌单里移除已不属于任何歌单的曲目时可直接从资料库彻底删除；批量管理统一为一套入口。新增全局多语言（简体中文 / 繁體中文 / English / 日本語）。
...
项目面向多种人群，界面文案固定支持三种语言：**简体中文 / 繁体中文 / English**。

- **新增界面文案必须同时补齐英文**：在 `frontend/src/core/i18n.js` 的 `I18N_MAP` 里给出英文翻译，缺英文即视为该功能未完成。
- **繁体中文由自动转换覆盖**：简体源文案经 `frontend/src/core/zhHant.js` 自动转为繁体，无需逐条手写；仅当需要「用词而非造字」的差异时，在其 `PHRASE` 表补充。
- **语言可在设置中随时切换**，并会持久化到 `localStorage(fufumidi_lang)` 与 `settings.lang`。首次安装启动按系统/浏览器语言自动设定；更新不重置用户选择。
- 验收「覆盖所有语言」= 简体文案存在 + `I18N_MAP` 有英文 + 繁体可自动转换。
