...
当前版本线：**3.4.0**（[发布说明](https://github.com/qdTXTbp/FuFumidi/releases/tag/v3.4.0)）。

3.4.0 要点：MuScriptor 转录稳定性修复——「节拍网格检测」权重纳入资源中心（国内镜像下载、统一存入软件目录，不再依赖国外源/外泄系统缓存），转录前可开关、未下载或失败时自动跳过，修复了 v3.3.1 转录失败的 `final0` 权重加载报错。
