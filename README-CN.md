...
当前版本线：**4.0.0**（[发布说明](https://github.com/qdTXTbp/FuFumidi/releases/tag/v4.0.0)）。

4.0.0 要点：新增云同步——用邮箱注册/登录账号（含人机验证），把歌单与 MIDI 曲目备份到 Cloudflare D1 + R2，并与手机端 FuMiVoice 双向同步；手动同步前可选择以本机还是云端存档为准。同时修复了数百首曲库一次性同步中途失败、歌单结构未上传、部分曲目只上传信息没有上传 MIDI 内容三个问题。
