...
4.3.0 要点：修复一批「一直默默影响体验」的问题——开启「启用音效」会让整个应用静音（均衡器 / 低音增强 / 空间声因此从未生效）、空间声把右侧声像压小而非展宽、播放速度方向与界面相反、极密曲目播放无声或丢音、接外部音源播放越久越卡、节拍器关不掉、音频引擎初始化失败后必须重启应用；可视化新增「沉浸模式」与音符瀑布纵深观感，滚动闪烁消除，大曲目帧时间与绘制开销显著下降；新手引导重做为按功能分类的 13 个章节（88 步，可只学需要的章节）。
