...
当前版本线：**3.3.1**（[发布说明](https://github.com/qdTXTbp/FuFumidi/releases/tag/v3.3.1)）。

3.3.1 要点：转录大幅提速——MuScriptor 改用 GPU 批量推理（长音频约 2.3× 提速，GPU 占用从约 68% 提升到约 90%），GPU 机器默认推荐改为「均衡」档（不再默认串行），新增「低音增强」开关（旋律密集时保留贝斯/低音声部）；修复：导入歌曲不再打断播放、钢琴卷帘滚轮方向、新手引导不再模糊高亮框内容、和弦分析支持七和弦/挂留/转位、播放列表支持双击播放、GPU 加速安装可取消、转录进度改为真实分块进度、去掉冗余的「转录中 3%」文字。
