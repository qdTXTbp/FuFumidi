...
3.3.0 要点：专业音效调节（10 段 EQ + 预设 + 低音增强/空间声）、播放模式与无缝衔接、断点续播/书签/睡眠定时、智能音乐库（艺术家/专辑/流派/文件夹整理、最近与最常播放、标签与封面编辑）、音频曲目播放、.lrc 自动加载、托盘后台播放与开机自启、「清除用户数据」按钮、Salamander 三角钢琴音色；修复并行转录不生效（多进程池，GPU 自动降路）、乐谱切换卡顿、大模型下载超时，模型与音色升级不再丢失。
