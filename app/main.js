...
function registerIpc() {
  ...
  // 视频导出：接收渲染器录制的 WebM（可视化 + 合成音频）→ 内置 ffmpeg 转码 MP4(H.264/AAC)
  ipcMain.handle('video:transcode', async (evt, opts) => {
    try {
      ...
      const code =
        ...
        'r = subprocess.run([ff, "-y", "-hide_banner", "-loglevel", "error", "-i", ' + JSON.stringify(webm) +
        ', "-c:v", "libx264", "-pix_fmt", "yuv420p", "-c:a", "aac", "-movflags", "+faststart", ' + JSON.stringify(out) +
        '], capture_output=True)\n' +
