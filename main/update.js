...
function registerUpdateIpc({ ipcMain, shell, BrowserWindow, app, path, fs, net }) {
  async function fetchLatestRelease() {
    ...
    for (const base of UPDATE_MIRRORS) {
      try {
        ...
        const r = await net.fetch(base, { headers: { 'user-agent': 'FuFumidi/3.1.14' }, signal: ctrl.signal });
        clearTimeout(to);
  ...
  async function fetchReleaseNotes(tag) {
    ...
    for (const u of endpoints) {
      try {
        const ctrl = new AbortController(); const to = setTimeout(() => ctrl.abort(), 12000);
        const r = await net.fetch(u, { headers: { 'user-agent': 'FuFumidi/3.1.14' }, signal: ctrl.signal });
        clearTimeout(to);
        if (!r.ok) { lastErr = new Error('HTTP ' + r.status); continue; }
        const d = await r.json();
        if (d && d.tag_name) return d;
