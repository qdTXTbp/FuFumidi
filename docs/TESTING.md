...
内测版不必每次跑完整清单，但下面三条不能省：

| 阶段 | 必跑 | 说明 |
|---|---|---|
| 内测发布（`X.Y.Z-beta.N` / `-rc.N`） | L1 / L2 全绿 + L3 子集（A 安装 / B 首启与数据目录 / I 增量更新） | 内测的价值是尽早暴露问题，不是省掉安装与更新验证 |
| 转正发布（`X.Y.Z`） | 完整 L3 清单 + L4 | 另须覆盖「上一个正式版 → 本次正式版」的直升级路径 |
| 每次发版 | 通道隔离回归 | 正式通道下「检查更新」看到任何 prerelease 版本号即为失败 |

内测版同样要归档报告（`docs/test-reports/YYYY-MM-DD-vX.Y.Z-beta.N-vm.md`），
未跑项必须在报告中显式声明。双通道的发布步骤见 `UPDATING.md` 第 5 节。
