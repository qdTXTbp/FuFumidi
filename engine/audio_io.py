...
def load_audio_float32(src, sr):
    ...
    注意：一律先经 decode_to_wav（soundfile 快速通道 / imageio-ffmpeg 静态 ffmpeg）
    解码，绝不依赖 librosa.load → audioread 的系统解码后端。打包后的应用若机器上
    未安装 ffmpeg，librosa 会抛 NoBackendError；此处则始终能走内置 ffmpeg。
    """
